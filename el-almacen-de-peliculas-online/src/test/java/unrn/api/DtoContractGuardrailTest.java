...
public class DtoContractGuardrailTest {
    ...
    void DetallePeliculaDTO_serialization_mandatoryFieldsAndTypes() {
        ...
        assertFalse(node.has("stockDisponible"), "public detail DTO must not expose stockDisponible");
        assertFalse(node.has("version"), "public detail DTO must not expose optimistic lock version");
    }

    @Test
    @DisplayName("AdminPeliculaDTO_serialization_includesStockAndVersion")
    void AdminPeliculaDTO_serialization_includesStockAndVersion() {
        AdminPeliculaDTO sample = new AdminPeliculaDTO(
                1L,
                "Titulo ejemplo",
                "NUEVO",
                List.of("Director 1"),
        ...
        assertTrue(node.has("stockDisponible"), "stockDisponible must be present");
        assertTrue(node.get("stockDisponible").isNumber(), "stockDisponible must be numeric");
        assertTrue(node.has("version"), "version must be present");
        assertTrue(node.get("version").isNumber(), "version must be numeric");
    }
}
