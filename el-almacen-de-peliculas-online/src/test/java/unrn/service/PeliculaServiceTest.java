...
class PeliculaServiceTest {
    ...
    void crear_dtoValido_creaPelicula() {
        ...
        assertEquals("Francis Ford Coppola", peliculaCreada.directores().get(0).nombre());
        assertEquals(2, peliculaCreada.actores().size());
        assertTrue(peliculaCreada.actores().stream().anyMatch(a -> a.nombre().equals("Marlon Brando")));
        assertTrue(peliculaCreada.actores().stream().anyMatch(a -> a.nombre().equals("Al Pacino")));
    }
